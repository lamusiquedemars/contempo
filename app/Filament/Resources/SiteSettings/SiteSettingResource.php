        ...
        return $schema
            ->components([
                ...
                KeyValue::make('social_links')
                    ...
                    ->valueLabel('URL')
                    ->columnSpanFull(),
                Toggle::make('audience_cron_enabled')
                    ->label('Activer le cron des messages ciblés')
                    ->default(true),
                TextInput::make('audience_send_limit')
                    ->label('Messages par passage')
                    ->numeric()
                    ->minValue(1)
                    ->maxValue(120)
                    ->default(25),
                TextInput::make('audience_send_domain_limit')
                    ->label('Messages par domaine et par passage')
                    ->numeric()
                    ->minValue(1)
                    ->maxValue(25)
                    ->default(3),
                TextInput::make('audience_send_max_seconds')
                    ->label('Durée maximum du passage')
                    ->numeric()
                    ->minValue(30)
                    ->maxValue(240)
                    ->default(180),
                TextInput::make('audience_send_max_attempts')
                    ->label('Tentatives maximum par destinataire')
                    ->numeric()
                    ->minValue(1)
                    ->maxValue(5)
                    ->default(3),
                Textarea::make('audience_excluded_domains')
                    ->label('Domaines exclus des envois ciblés')
                    ->helperText('Séparez les domaines par des virgules.')
                    ->columnSpanFull(),
                TextInput::make('audience_cron_token')
                    ->label('Jeton cron')
                    ->disabled()
                    ->dehydrated()
                    ->columnSpanFull(),
                TextInput::make('audience_cron_url')
                    ->label('URL cron à appeler chez l’hébergeur')
                    ->default(fn (?SiteSetting $record): ?string => $record?->audienceCronUrl())
                    ->disabled()
                    ->dehydrated(false)
                    ->columnSpanFull(),
            ]);
