...
class InstrumentProjectionController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        $data = $request->validate(['id' => ['required', 'integer'], 'reference' => ['nullable', 'string', 'max:80'], 'name' => ['required', 'string', 'max:255'], 'family' => ['nullable', 'string', 'max:80'], 'maker' => ['nullable', 'string', 'max:255'], 'description' => ['nullable', 'string'], 'sale_amount' => ['nullable', 'numeric'], 'rental_amount' => ['nullable', 'numeric'], 'availability' => ['required', 'string', 'in:available,reserved,rented,in_workshop,sold,archived'], 'published_at' => ['required', 'date']]);
